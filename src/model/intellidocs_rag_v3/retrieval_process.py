...
class Retriever:
    def __init__(
        self, embeddings_df_path: str, model_name: str = sent_tokenizer_model_name
    ):
    ...
    def _prepare_embeddings(self) -> torch.Tensor:
        return torch.tensor(
            np.array(self.embeddings_df["embeddings"].tolist()), dtype=torch.float32
        ).to(self.device)

    def retrieve_relevant_resources(
        self, query: str, n_resources_to_return: int = 5
